...
class StringPreHandler:
    ...
    def numberTranslator(cls, target):
        ...
        pattern = re.compile(u"[零〇一壹幺二贰两三叁四肆五伍六陆七柒八捌九玖]")
        match = pattern.finditer(target)
        for m in match:
        ...
        pattern = re.compile(u"(?<!(周|星期))0?[0-9]?[十拾][0-9]?")
        match = pattern.finditer(target)
        for m in match:
            group = m.group()
            s = re.split(u"[十拾]", group)
            num = 0
        ...
        pattern = re.compile(u"0?[1-9][百佰][0-9]?[0-9]?")
        match = pattern.finditer(target)
        for m in match:
            group = m.group()
            s = re.split(u"[百佰]", group)
            s = list(filter(None, s))
            num = 0
            if len(s) == 1:
                hundred = int(s[0])
        ...
        pattern = re.compile(u"0?[1-9][千仟][0-9]?[0-9]?[0-9]?")
        match = pattern.finditer(target)
        for m in match:
            group = m.group()
            s = re.split(u"[千仟]", group)
            s = list(filter(None, s))
            num = 0
            if len(s) == 1:
                thousand = int(s[0])
    ...
    def wordToNumber(cls, s):
        ...
        """
        if (s == u'零') or (s == '0') or (s == u'〇'):
            return 0
        elif (s == u'一') or (s == '1') or (s == u'壹') or (s == u'幺'):
            return 1
        elif (s == u'二') or (s == u'两') or (s == '2') or (s == u'贰'):
            return 2
            ...
            return 6
        elif (s == u'七') or (s == u'天') or (s == u'日') or (s == u'末') or (s == '7') or (s == u'柒'):
            return 7
